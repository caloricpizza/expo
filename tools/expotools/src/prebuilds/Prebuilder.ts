...
import { Flavor, Framework, XcodebuildOptions } from './XcodeProject.types';
...
export async function prebuildPackageAsync(
  ...
  if (canPrebuildPackage(pkg)) {
    ...
    await buildFrameworksForProjectAsync(xcodeProject, options);
...
export async function buildFrameworksForProjectAsync(
  ...
  await xcodeProject.buildXcframeworkAsync(frameworks, options);
}
